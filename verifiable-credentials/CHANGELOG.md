...
- **Major:** Migration of VC issuance to GitLab with a new CI pipeline and chart conformance, enhancing build and deployment processes.
- Improvements to the Certify schema creation process, now utilizing an init container instead of a post-install hook, and ensuring keystore backup to a Secret.
- Updates to the issuer identity management, including clarifications on keystore installation options and fixes to startup issues by removing the mock VCI plugin and adding issuer display.
- Cleanup of unused components and scoping of the keystore-backup permission to its own ServiceAccount for better security and maintainability.
- Helm chart added alongside several updates for phase 1 implementations, streamlining deployment configurations.
