...
## g2p-bridge/g2p-bridge 1.2.0 — 2026-07-19
...
- Removed the legacy Keymanager crypto backend, eliminating dead signing code and associated dependencies.  
- Changed the SPAR build dependency from GitHub openg2p-spar@2.0 to GitLab spar@develop for consistency.  
- Updated the FastAPI version to 'develop' for uniformity across builds, while also pinning down a specific version for creating a frozen version.  
- Fixed the mapper initialization order in connectors to ensure proper functionality.  
- Moved connectors to g2p-bridge-connectors, allowing Celery to pull them by Git reference.  
